...
const DELAY_COLOR: Record<string, string> = { Normal: "#4ADE80", Moderate: "#FBBF24", High: "#EF4444", Critical: "#DC2626" };
const RISK_COLOR: Record<string, string>  = { Low: "#4ADE80", Medium: "#FBBF24", High: "#EF4444" };
...
export default function DelayCards({ crossings }: { crossings: Crossing[] }) {
  ...
  const Card = ({ c }: { c: Crossing }) => {
    const delayColor = DELAY_COLOR[c.delay_label ?? "Normal"] ?? "#4ADE80";
    const riskColor  = RISK_COLOR[c.risk_level]  ?? "#4ADE80";
    const waitVsAvg  = c.wait_vs_avg ?? 0;

    return (
      <div className="rounded-xl p-4 transition-all hover:scale-[1.01]"
        style={{ border: `1px solid ${delayColor}40`, background: "#030712" }}>
        <div className="flex items-start justify-between gap-2 mb-3">
          ...
          <span className="text-xs px-2 py-0.5 rounded-lg font-semibold flex-shrink-0"
            style={{ background: `${delayColor}18`, color: delayColor, border: `1px solid ${delayColor}` }}>
            {c.delay_label ?? "Normal"}
          </span>
        </div>
        <div className="grid grid-cols-3 gap-2">
          {[
            { label: "Wait",       value: `${c.wait_time}m`,                              color: delayColor },
            { label: "vs Avg",     value: `${waitVsAvg > 0 ? "+" : ""}${waitVsAvg}%`,    color: waitVsAvg > 0 ? "#EF4444" : "#4ADE80" },
        ...
        <div className="mt-3 flex items-center justify-between text-xs">
          ...
          <span className="font-semibold" style={{ color: riskColor }}>{c.risk_level} Risk</span>
        </div>
        <div className="mt-2 w-full rounded-full" style={{ height: 3, background: "#1F2937" }}>
          <div style={{ width: `${Math.min((c.wait_time / 200) * 100, 100)}%`, height: 3, background: delayColor, borderRadius: 9999 }} />
        </div>
      </div>
    );
  };
